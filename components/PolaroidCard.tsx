...
const DevelopingIndicator = () => (
    <div className="flex flex-col items-center justify-center h-full bg-neutral-800 text-neutral-400 relative overflow-hidden">
        {/* Shimmer effect */}
        <motion.div
            className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-neutral-800 via-neutral-700 to-neutral-800"
            initial={{ x: "-100%" }}
            animate={{ x: "100%" }}
            transition={{
                duration: 2.5,
        ...
        <div className="z-10 text-center">
            <svg className="animate-spin h-8 w-8 text-neutral-500 mx-auto mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="font-permanent-marker text-lg">Developing...</p>
        </div>
    </div>
);

const ErrorDisplay = ({ onRetry }: { onRetry?: () => void }) => (
    <div className="flex flex-col items-center justify-center h-full text-center p-4 bg-neutral-800 text-neutral-300">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-red-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <p className="font-permanent-marker text-lg mb-4">Generation Failed</p>
        {onRetry && (
            <button
                onClick={onRetry}
                className="font-permanent-marker text-base text-center text-black bg-yellow-400 py-2 px-5 rounded-sm transform transition-transform duration-200 hover:scale-105 hover:-rotate-2 hover:bg-yellow-300 shadow-[1px_1px_0px_1px_rgba(0,0,0,0.2)]"
            >
                Retry
            </button>
        )}
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, onZoom, isMobile }) => {
        ...
        <>
            <div className="w-full bg-neutral-900 shadow-inner flex-grow relative overflow-hidden">
                <AnimatePresence mode="wait">
                    {status === 'pending' && (
                        <motion.div key="pending" exit={{ opacity: 0 }} className="w-full h-full">
                            <DevelopingIndicator />
                        </motion.div>
                    )}
                    {status === 'error' && (
                        <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full">
                            <ErrorDisplay onRetry={onShake ? () => onShake(caption) : undefined} />
                        </motion.div>
                    )}
                    ...
                    {status === 'done' && !imageUrl && (
                        <motion.div key="placeholder" className="w-full h-full group">
                            <Placeholder />
                        </motion.div>
                    )}
                </AnimatePresence>
    ...
    if (isMobile) {
        return (
            <motion.div 
                className="bg-neutral-100 dark:bg-neutral-100 !p-4 !pb-16 flex flex-col items-center justify-start aspect-[3/4] w-80 max-w-full rounded-md shadow-lg relative"
