...
async function callGeminiWithRetry(imagePart: object, textPart: object): Promise<GenerateContentResponse> {
    const maxRetries = 3;
    const initialDelay = 20000; // Increased to 20 seconds for more patient retries against rate limits.
...
export async function generateStyledImage(imageDataUrl: string, prompt: string): Promise<string> {
  ...
  const [, mimeType, base64Data] = match;
    ...
    try {
        console.log("Attempting image generation with prompt...");
        ...
        if (errorMessage.includes("The AI model responded with text instead of an image")) {
             throw new Error(`The AI was unable to generate an image for this style, possibly due to a safety policy. Please try a different photo or style.`);
        }
